            ...
            <div key={c.id}><div className="bg-slate-900/80 backdrop-blur-md rounded-xl border border-white/10 p-4 hover:border-[#229DD8]/20 transition-colors">
              <div className="flex items-start gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-sm text-slate-300 leading-relaxed" style={{lineHeight: "1.65", maxHeight: c.comment_text && c.comment_text.length > 600 && !expandedCards[c.id] ? "260px" : "none", overflow: "hidden", position: "relative"}}>{c.parent_text ? (<><div className="bg-slate-950/40 rounded-lg p-3 mb-2 border-l-2 border-slate-600/30"><div className="text-xs text-slate-500 mb-1">{c.parent_author || "User"}</div><div className="text-sm text-slate-400 leading-relaxed">{stripHtml(c.parent_text)}</div></div><div className="text-sm text-slate-200 leading-relaxed">{stripHtml(c.comment_text)}</div></>) : (c.signal_type === "cycle_log" ? formatComment(c.comment_text) : stripHtml(c.comment_text))}{c.comment_text && c.comment_text.length > 600 && !expandedCards[c.id] && (<div style={{position:"absolute",bottom:0,left:0,right:0,height:"80px",background:"linear-gradient(180deg, transparent 0%, rgba(15,23,36,0.95) 100%)",pointerEvents:"none"}} />)}</div>{c.comment_text && c.comment_text.length > 600 && (<button onClick={() => setExpandedCards(prev => ({...prev, [c.id]: !prev[c.id]}))} className="text-xs text-[#229DD8] hover:text-cyan-300 mt-1 font-medium cursor-pointer">{expandedCards[c.id] ? "Show less" : "Show more"}</button>)}
