            ...
            <div key={c.id}><div className="bg-slate-900/80 backdrop-blur-md rounded-xl border border-white/10 p-4 hover:border-[#229DD8]/20 transition-colors">
              <div className="flex items-start gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mb-2">
                    <div className="font-medium text-slate-400 text-xs mb-1.5">{c.author_name}<span className="text-slate-600 mx-1.5">•</span>{c.published_at ? new Date(c.published_at).toLocaleDateString() : ""}</div>
                    <div className="flex flex-wrap gap-1.5">{c.signal_types && c.signal_types.filter(t => t !== "general" && t !== "noise" && t !== "admin_update").map(t => (<span key={t} className={"px-2 py-0.5 rounded text-[10px] uppercase tracking-wide font-semibold " + (t === "cycle_log" ? "bg-[#229DD8]/15 text-[#229DD8] border border-[#229DD8]/20" : t === "side_effect" ? "bg-amber-500/15 text-amber-400 border border-amber-500/20" : t === "benefit" ? "bg-emerald-500/15 text-emerald-400 border border-emerald-500/20" : t === "travis_reply" ? "bg-cyan-500/15 text-cyan-300 border border-cyan-500/20" : t === "question" ? "bg-slate-500/15 text-slate-400 border border-slate-500/20" : t === "verified_override" ? "bg-purple-500/15 text-purple-300 border border-purple-500/30" : "bg-slate-800/50 text-slate-500")}>{t.replace("_", " ")}</span>))}</div>
                  </div>
                  <div className="text-sm text-slate-300 leading-relaxed" style={{lineHeight: "1.65", maxHeight: c.comment_text && c.comment_text.length > 600 && !expandedCards[c.id] && filter !== "cycle_log" ? "260px" : "none", overflow: "hidden", position: "relative"}}>{c.parent_text ? (<><div className="bg-slate-950/40 rounded-lg p-3 mb-2 border-l-2 border-slate-600/30"><div className="text-xs text-slate-500 mb-1">{c.parent_author || "User"}</div><div className="text-sm text-slate-400 leading-relaxed">{stripHtml(c.parent_text)}</div></div><div className="text-sm text-slate-200 leading-relaxed">{stripHtml(c.comment_text)}</div></>) : (c.signal_type === "cycle_log" ? formatComment(c.comment_text) : stripHtml(c.comment_text))}{c.comment_text && c.comment_text.length > 600 && !expandedCards[c.id] && filter !== "cycle_log" && (<div style={{position:"absolute",bottom:0,left:0,right:0,height:"80px",background:"linear-gradient(180deg, transparent 0%, rgba(15,23,36,0.95) 100%)",pointerEvents:"none"}} />)}</div>{c.comment_text && c.comment_text.length > 600 && filter !== "cycle_log" && (<button onClick={() => { const wasExpanded = expandedCards[c.id]; setExpandedCards(prev => ({...prev, [c.id]: !prev[c.id]})); if (wasExpanded && expandedReplies[c.comment_id]) { setExpandedReplies(prev => ({...prev, [c.comment_id]: false})); } }} className="text-xs text-[#229DD8] hover:text-cyan-300 mt-1 font-medium cursor-pointer">{expandedCards[c.id] ? "Show less" : "Show more"}</button>)}
