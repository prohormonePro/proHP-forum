          ...
          <div className="flex gap-3">
            ...
            <div className="flex-1 min-w-0">
              {!isCollapsed && (<>
                ...
                {p.image_url && !p.is_deleted && (<div className="mb-2"><img src={p.image_url} alt="" className="max-w-full max-h-96 rounded-lg border border-white/5" loading="lazy" /></div>)}
                <div className="flex items-center gap-1 flex-wrap">
                  ...
                  {user && (user.id === p.author_id || user.tier === 'admin') && !p.is_deleted && (<button onClick={() => { if (confirm('Delete this comment?')) deletePost.mutate({ postId: p.id }); }} className="flex items-center gap-1 px-2 py-1 text-[11px] text-slate-600 hover:text-red-400 hover:bg-red-500/5 rounded-md transition-all"><Trash2 className="w-3 h-3" /></button>)}
