          ...
          <div className="flex gap-3">
            ...
            <div className="flex-1 min-w-0">
              {!isCollapsed && (<>
                ...
                {p.image_url && !p.is_deleted && (<div className="mb-2"><details open className="rounded-lg overflow-hidden border border-white/10"><summary className="text-[10px] text-[#229DD8] cursor-pointer p-2 hover:bg-slate-800/30 flex items-center gap-1.5 select-none"><svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" /></svg>{isVideoFile(p.image_url) ? "Video attachment" : p.image_url.endsWith(".pdf") ? "PDF document" : "Image attachment"}</summary><div>{isVideoFile(p.image_url) ? (<video src={p.image_url} controls playsInline preload="metadata" className="w-full max-h-[400px] bg-black rounded-lg" />) : p.image_url.endsWith('.pdf') ? (<a href={p.image_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-[#229DD8] hover:text-white text-sm p-3">View PDF</a>) : (<img src={p.image_url} alt="" className="w-full max-h-[400px] object-contain rounded-lg" loading="lazy" />)}</div></details></div>)}
