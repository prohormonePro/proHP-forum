        ...
        {isLoading ? (
          ...
          data?.compounds?.map((c) => (
            <Link
              key={c.slug}
              to={`/compounds/${c.slug}`}
              className="prohp-card px-4 py-2 md:py-4 hover:bg-slate-800/40 transition-colors group w-full overflow-hidden"
            >
              <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-base md:text-lg font-semibold text-slate-200 group-hover:text-prohp-400 transition-colors truncate">
                      {c.name}
                    </span>
