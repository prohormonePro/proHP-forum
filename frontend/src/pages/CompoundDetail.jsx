...
import { ChevronLeft, MessageSquare, Search, X, AlertTriangle, Youtube, Lock, ExternalLink, ArrowUp, ArrowDown, CornerDownRight, CheckCircle, Award, Clock, Shield, Beaker, Heart, Activity, Zap, Droplets } from 'lucide-react';
...
function getSessionInt(key, fallback) {
  try { var raw = sessionStorage.getItem(key); var v = raw ? parseInt(raw, 10) : fallback; return Number.isFinite(v) ? v : fallback; } catch (e) { return fallback; }
}
...
function extractYouTubeId(input) {
  ...
  try {
    var u = new URL(s); var host = u.hostname.replace('www.', '');
    if (host === 'youtu.be') { var id = u.pathname.replace('/', '').trim(); return /^[a-zA-Z0-9_-]{11}$/.test(id) ? id : ''; }
    var v = u.searchParams.get('v'); if (v && /^[a-zA-Z0-9_-]{11}$/.test(v)) return v;
    var m = u.pathname.match(/\/embed\/([a-zA-Z0-9_-]{11})/); if (m && m[1]) return m[1];
  } catch (e) {} return '';
}
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-5xl rounded-2xl overflow-hidden border border-white/10 bg-slate-950 shadow-2xl">
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
          ...
          <button onClick={onClose} className="text-slate-400 hover:text-slate-200 inline-flex items-center gap-2 text-xs px-2 py-1 rounded-lg hover:bg-white/5"><X className="w-4 h-4" /> Close</button>
        </div>
...
function legalClass(status) {
  ...
  if (s === 'active') return 'bg-emerald-900/60 text-emerald-300 border border-emerald-700/40';
  if (s === 'banned') return 'bg-red-900/60 text-red-300 border border-red-700/40';
  if (s === 'grey_market') return 'bg-yellow-900/60 text-yellow-300 border border-yellow-700/40';
  return 'bg-slate-800 text-slate-300';
}
function legalLabel(status) {
  ...
  if (s === 'grey_market') return 'Grey Market';
  return 'Active';
}

/* ═══════════════════════════════════════════
   V3 SIGNATURE COMPONENT: RISK METER
   ═══════════════════════════════════════════ */
...
function riskBarTextColor(level) {
...
function getDefaultLevels(category, riskTier) {
  ...
  var tier = (riskTier || '').toLowerCase();
  if (cat === 'ancillary' || cat === 'natural' || cat === 'other') return null;
  if (cat === 'sarm') {
    if (tier === 'extreme') return { s: 5, l: 3, h: 4, w: 1, c: 4, m: 4 };
    if (tier === 'high') return { s: 4, l: 1, h: 3, w: 1, c: 3, m: 3 };
    if (tier === 'moderate') return { s: 3, l: 1, h: 2, w: 1, c: 2, m: 2 };
    return { s: 2, l: 0, h: 1, w: 0, c: 1, m: 1 };
  }
  if (cat === 'prohormone') {
    if (tier === 'extreme') return { s: 5, l: 4, h: 4, w: 3, c: 4, m: 4 };
    if (tier === 'high') return { s: 4, l: 2, h: 2, w: 3, c: 2, m: 3 };
    if (tier === 'moderate') return { s: 3, l: 1, h: 2, w: 2, c: 2, m: 2 };
    return { s: 2, l: 1, h: 1, w: 2, c: 1, m: 1 };
  }
  if (cat === 'peptide') return { s: 1, l: 0, h: 0, w: 1, c: 0, m: 1 };
  if (cat === 'serm') return { s: 0, l: 1, h: 0, w: 0, c: 0, m: 2 };
  if (cat === 'ai') return { s: 0, l: 1, h: 0, w: 0, c: 0, m: 1 };
          ...
          return (
            <div key={bar.key}>
              ...
              <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
                <div className={'h-full rounded-full transition-all duration-700 ease-out ' + riskBarColor(bar.level)} style={{ width: pct + '%' }} />
              </div>
              {bar.key === 'suppression' && bar.note && (
                <div className="mt-1 text-[11px] text-slate-500 italic">
                  Your body slows testosterone production. Moody, tired, unmotivated. <Link to="/compounds/arimiplex" className="text-prohp-400 hover:text-prohp-300">PCT</Link> fixes this.
                </div>
              )}
              {bar.key === 'mood' && bar.note && (
                <div className="mt-1 text-[11px] text-slate-500 italic">
                  DHT slows <Link to="/compounds/comt" className="text-prohp-400 hover:text-prohp-300">COMT</Link> enzyme. Anxiety, irritability, fight-or-flight.
                </div>
              )}
            </div>
          );
...
/* ═══════════════════════════════════════════
   V3: TEST BASE SECTION
   ═══════════════════════════════════════════ */

function TestBaseSection({ compound }) {
  var cat = (compound.category || '').toLowerCase();
  var tier = (compound.risk_tier || '').toLowerCase();
  ...
  /* Check if compound IS a test base (4-Andro products) */
  var slug = (compound.slug || '').toLowerCase();
  var isTestBase = ['andriol', 'sustanon-250', 'brutal-4ce'].indexOf(slug) !== -1;
  if (isTestBase) return null;

  return (
    <div className="prohp-card p-5 mb-4 border border-orange-700/20 bg-orange-900/[0.04]">
      <div className="flex items-center gap-2 mb-3">
        <AlertTriangle className="w-4 h-4 text-orange-400" />
        <span className="text-sm font-bold text-slate-200">Test Base</span>
        <span className="text-[10px] font-bold text-orange-400 uppercase tracking-wider">Non-negotiable</span>
      </div>
      <div className="mb-3">
        <div className="text-xs text-slate-400 mb-2">OTC (over-the-counter):</div>
        <div className="flex flex-wrap gap-2">
          <Link to="/compounds/andriol" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800 border border-white/10 text-xs font-semibold text-slate-200 hover:border-prohp-400/40 hover:text-prohp-400 transition-all">
            Andriol <span className="text-[9px] text-slate-500">(most popular)</span>
          </Link>
          <Link to="/compounds/sustanon-250" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800 border border-white/10 text-xs font-semibold text-slate-200 hover:border-prohp-400/40 hover:text-prohp-400 transition-all">
            Sustanon 250
          </Link>
          <Link to="/compounds/brutal-4ce" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-800 border border-white/10 text-xs font-semibold text-slate-200 hover:border-prohp-400/40 hover:text-prohp-400 transition-all">
            Brutal 4ce
          </Link>
        </div>
      </div>
      <div className="text-[11px] text-slate-500 leading-relaxed">
        Without a test base, you get: dry joints, flat mood, low libido, and deeper suppression. 4-Andro provides the testosterone and estrogen your body needs while running {compound.name}.
      </div>
    </div>
  );
}
  ...
  return (
    <div className="prohp-card p-5 mb-4 border border-prohp-400/10 bg-prohp-400/[0.02]">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-sm font-bold text-slate-200">Must-Have Ancillaries</span>
      </div>
      <div className="space-y-3">
        {/* Daily Essentials - always */}
        <div>
          <div className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider mb-1.5">Daily Essentials</div>
          <div className="flex flex-wrap gap-1.5">
            <Link to="/compounds/vitamin-d" className="text-[11px] px-2.5 py-1 rounded-md bg-emerald-900/30 border border-emerald-700/30 text-emerald-300 hover:border-emerald-500/50 transition-colors">Vitamin D 5000 IU</Link>
            <Link to="/compounds/fish-oil" className="text-[11px] px-2.5 py-1 rounded-md bg-emerald-900/30 border border-emerald-700/30 text-emerald-300 hover:border-emerald-500/50 transition-colors">Fish Oil</Link>
            <Link to="/compounds/magnesium" className="text-[11px] px-2.5 py-1 rounded-md bg-emerald-900/30 border border-emerald-700/30 text-emerald-300 hover:border-emerald-500/50 transition-colors">Magnesium</Link>
            <Link to="/compounds/zinc" className="text-[11px] px-2.5 py-1 rounded-md bg-emerald-900/30 border border-emerald-700/30 text-emerald-300 hover:border-emerald-500/50 transition-colors">Zinc 25mg</Link>
          </div>
        </div>

        {/* On-Cycle Liver - if methylated */}
        {isMethylated && (
          <div>
            <div className="text-[10px] font-bold text-yellow-400 uppercase tracking-wider mb-1.5">On-Cycle Liver Support <span className="text-yellow-500/60">(methylated)</span></div>
            <div className="flex flex-wrap gap-1.5">
              <Link to="/compounds/tudca" className="text-[11px] px-2.5 py-1 rounded-md bg-yellow-900/20 border border-yellow-700/30 text-yellow-300 hover:border-yellow-500/50 transition-colors">TUDCA 1000mg</Link>
              <Link to="/compounds/nac" className="text-[11px] px-2.5 py-1 rounded-md bg-yellow-900/20 border border-yellow-700/30 text-yellow-300 hover:border-yellow-500/50 transition-colors">NAC 600mg</Link>
            </div>
          </div>
        )}

        {/* COMT Support - if DHT converting */}
        {isDHT && (
          <div>
            <div className="text-[10px] font-bold text-amber-400 uppercase tracking-wider mb-1.5">COMT Support <span className="text-amber-500/60">(DHT compound)</span></div>
            <div className="flex flex-wrap gap-1.5">
              <Link to="/compounds/magnesium" className="text-[11px] px-2.5 py-1 rounded-md bg-amber-900/20 border border-amber-700/30 text-amber-300 hover:border-amber-500/50 transition-colors">Mag Glycinate 400mg</Link>
              <Link to="/compounds/zinc" className="text-[11px] px-2.5 py-1 rounded-md bg-amber-900/20 border border-amber-700/30 text-amber-300 hover:border-amber-500/50 transition-colors">Zinc 50mg</Link>
              <Link to="/compounds/p5p-b6" className="text-[11px] px-2.5 py-1 rounded-md bg-amber-900/20 border border-amber-700/30 text-amber-300 hover:border-amber-500/50 transition-colors">P5P 100mg</Link>
              <Link to="/compounds/coq10-ubiquinol" className="text-[11px] px-2.5 py-1 rounded-md bg-amber-900/20 border border-amber-700/30 text-amber-300 hover:border-amber-500/50 transition-colors">CoQ10 200mg</Link>
            </div>
          </div>
        )}

        {/* Arimistane note - if 4-Andro */}
        {is4Andro && (
          <div>
            <div className="text-[10px] font-bold text-blue-400 uppercase tracking-wider mb-1.5">Estrogen Management</div>
            <div className="flex flex-wrap gap-1.5 mb-1">
              <Link to="/compounds/arimistane" className="text-[11px] px-2.5 py-1 rounded-md bg-blue-900/20 border border-blue-700/30 text-blue-300 hover:border-blue-500/50 transition-colors">Arimistane (have on hand)</Link>
            </div>
            <div className="text-[10px] text-slate-500 leading-snug">Only take if water retention, puffy/itchy nipples, or moodiness appears. Do NOT take preventatively. Estrogen is anabolic and supports mood.</div>
          </div>
        )}
        ...
        {isSuppressive && (
          <div>
            <div className="text-[10px] font-bold text-red-400 uppercase tracking-wider mb-1.5">PCT (Post-Cycle Therapy)</div>
            <div className="flex flex-wrap gap-1.5">
              <Link to="/compounds/arimiplex" className="text-[11px] px-2.5 py-1 rounded-md bg-red-900/20 border border-red-700/30 text-red-300 hover:border-red-500/50 transition-colors">Arimiplex (4 weeks)</Link>
              <Link to="/compounds/enclomiphene" className="text-[11px] px-2.5 py-1 rounded-md bg-red-900/20 border border-red-700/30 text-red-300 hover:border-red-500/50 transition-colors">Enclomiphene (longer cycles)</Link>
            </div>
          </div>
        )}
      </div>
...
/* ═══════════════════════════════════════════
   REMAINING V2 COMPONENTS (preserved)
   ═══════════════════════════════════════════ */
...
function HalfLifeBar({ halfLife, dosageRange }) {
  ...
  return (
    <div className="prohp-card p-4 mb-4 flex flex-wrap items-center gap-4 border border-white/5">
      {halfLife && (<div className="flex items-center gap-2"><Clock className="w-4 h-4 text-prohp-400" /><span className="text-xs text-slate-400">Half-life:</span><span className="text-xs font-bold text-slate-200">{halfLife}</span></div>)}
      {dosageRange && (<div className="flex items-center gap-2"><Beaker className="w-4 h-4 text-prohp-400" /><span className="text-xs text-slate-400">Dose:</span><span className="text-xs font-bold text-slate-200">{dosageRange}</span></div>)}
    </div>
  );
}
...
function BloodworkCTA({ compoundName }) {
  ...
  return (
    <div className="prohp-card p-5 mb-4 border border-prohp-400/15 bg-prohp-400/[0.03]">
      <div className="flex items-center gap-2 mb-3"><Activity className="w-4 h-4 text-prohp-400" /><span className="text-sm font-bold text-slate-200">Bloodwork</span></div>
      ...
      <a href={ultaUrl} target="_blank" rel="noopener noreferrer" className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-prohp-400 to-[#00c6ff] px-5 py-3 text-sm font-bold text-white shadow-lg hover:-translate-y-0.5 transition-all">
        <Activity className="w-4 h-4" /> Order your bloodwork. Walk into any lab. No doctor needed.
      </a>
    </div>
  );
}
  ...
  return (
    <div className="prohp-card p-5 mb-4 border border-emerald-700/20 bg-emerald-900/[0.06]">
      ...
      <div className="flex flex-wrap gap-3">
        <Link to="/cycles" className="prohp-btn-primary text-xs inline-flex items-center gap-1.5"><Zap className="w-3.5 h-3.5" /> Start a Cycle Log</Link>
        <Link to="/rooms/lab" className="text-xs text-slate-400 hover:text-prohp-400 transition-colors inline-flex items-center gap-1">Your log appears publicly in The Lab &rarr;</Link>
      </div>
    </div>
  );
}
...
function GateCTA({ gate_state, upgrade_cta }) {
  ...
  return (
    <div className="prohp-card p-6 mb-4 border border-[rgba(34,157,216,0.2)] bg-[rgba(34,157,216,0.04)]">
      <div className="flex items-center gap-2 mb-2"><Lock className="w-4 h-4 text-[#229DD8]" /><span className="text-sm font-bold text-slate-100">{isWindow ? "Unlock the Full Breakdown" : "Want the full breakdown?"}</span></div>
      <p className="text-[13px] text-slate-400 leading-relaxed mb-4">{isWindow ? upgrade_cta : "Dosing protocols, stacking logic, PCT, bloodwork markers."}</p>
      {isWindow ? (<a href="/compounds" className="prohp-btn-primary inline-flex items-center gap-2 text-xs px-4 py-2">Enter Your Email to Unlock</a>) : (<Link to="/register" className="inline-flex items-center justify-center rounded-xl bg-gradient-to-r from-[#0070f3] to-[#00c6ff] px-5 py-2.5 text-xs font-bold text-white shadow-lg hover:-translate-y-0.5 transition-all">Unlock Inner Circle</Link>)}
    </div>
  );
}
...
function DiscountSection({ compound, gate_state }) {
  ...
  if (isSoma) {
    return (
      <div className="bg-emerald-900/20 border border-emerald-700/30 rounded-xl p-4 mt-4">
        <div className="flex items-center gap-2 mb-2"><span className="text-emerald-400 text-sm font-semibold">Discount Available</span></div>
  ...
  return (
    <div className="bg-emerald-900/20 border border-emerald-700/30 rounded-xl p-4 mt-4">
      <div className="flex items-center gap-2 mb-2"><span className="text-emerald-400 text-sm font-semibold">Discount Available</span></div>
      <div className="flex items-center gap-3 flex-wrap">
        <code className="bg-slate-800 text-emerald-300 px-3 py-1.5 rounded-lg text-sm font-mono font-bold tracking-wider">{gate_state === 'member' ? activeMemCode : 'TRAVISD'}</code>
        <span className="text-slate-400 text-sm">{gate_state === 'member' ? '20% off - Inner Circle exclusive' : '10% off - use at checkout'}</span>
        <a href={compound.product_url} target="_blank" rel="noopener noreferrer" className="ml-auto inline-flex items-center gap-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold px-4 py-2 transition-colors shadow-md"><ExternalLink className="w-3.5 h-3.5" />Buy Now</a>
      </div>
      <p className="text-xs text-slate-500 mt-2">Apply to your order at the product page.</p>
      {gate_state !== 'member' && (<div className="mt-2 text-[11px] text-slate-500">Inner Circle members get 20% off.</div>)}
    </div>
  );
}
...
export default function CompoundDetail() {
  ...
  var createReply046 = useMutation({
    ...
    onSuccess: function() { queryClient.invalidateQueries({ queryKey: ["compound-thread", compound ? compound.thread_id : null] }); setReplyBody046(""); setReplyTo046(null); setReplyError046(""); },
  ...
  var handleReply046 = function(e) {
    ...
    setReplyError046("");
    createReply046.mutate({ thread_id: compound.thread_id, body: replyBody046.trim(), parent_id: replyTo046 || undefined });
  };
  ...
  useEffect(function() { setQ(''); setResults(null); setSearchErr(''); setSearchCountState(getSessionInt(searchKey, 0)); setBannerDismissed(getSessionInt(dismissKey, 0) === 1); }, [slug, searchKey, dismissKey]);
  ...
  function runSearch(e) {
    ...
    api.get('/api/threads/search/query?q=' + encodeURIComponent(query) + '&limit=12&offset=0')
      ...
      .catch(function(err2) { if (err2 && err2.name === 'AbortError') return; setSearchErr(err2 ? (err2.message || 'Search failed') : 'Search failed'); })
  ...
  return (
    <div className="max-w-3xl mx-auto animate-fade-in px-4 py-6">
      ...
      <div className="prohp-card p-6 mb-4">
        <div className="flex items-start justify-between gap-4 flex-wrap mb-4">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {compound.legal_status && compound.legal_status !== 'active' && (<span className={'text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded ' + legalClass(compound.legal_status)}>{legalLabel(compound.legal_status)}</span>)}
              {compound.legal_status === 'active' && (<span className={'text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded ' + legalClass('active')}>Active</span>)}
          ...
          <div className="flex gap-2">
            {videoId && (<button type="button" onClick={function() { setVideoOpen(true); }} className="prohp-btn-primary inline-flex items-center gap-2 text-xs"><Youtube className="w-4 h-4" /> Watch breakdown</button>)}
          </div>
        </div>
        ...
        {compound.product_url && (
          <div className="mt-3 pt-3 border-t border-white/5">
            <a href={compound.product_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs text-slate-400 hover:text-prohp-400 transition-colors"><ExternalLink className="w-3.5 h-3.5" /> Support the encyclopedia</a>
      ...
      <Modal open={videoOpen} title={(compound.name || 'Video') + ' - Breakdown'} onClose={function() { setVideoOpen(false); }}>
        <div className="aspect-video rounded-xl overflow-hidden bg-black/40 border border-white/10"><YouTubeEmbed videoId={videoId} title={compound.name + ' breakdown'} className="w-full h-full" /></div>
      </Modal>
      ...
      {compound.benefits && (
        <div className="prohp-card p-6 mb-4">
          <div className="text-sm font-semibold text-slate-200 mb-2">Benefits</div>
      ...
      {/* ═══ 7. MUST-HAVE ANCILLARIES ═══ */}
      <AncillariesSection compound={compound} />

      {/* ═══ 8. VIDEO ═══ */}
      {videoId && (
        <div className="prohp-card p-5 mb-4">
          <div className="flex items-center gap-2 mb-3"><Youtube className="w-4 h-4 text-red-500" /><span className="text-sm font-semibold text-slate-200">{compound.name} - Video Breakdown</span></div>
          <div className="aspect-video rounded-lg overflow-hidden bg-black/30 border border-white/5"><YouTubeEmbed videoId={videoId} title={compound.name + ' breakdown'} className="w-full h-full" /></div>
        </div>
      )}
      ...
      {compound.side_effects && (
        <div className="prohp-card p-6 mb-4">
          <div className="text-sm font-semibold text-slate-200 mb-2 flex items-center gap-1"><AlertTriangle className="w-4 h-4 text-yellow-500" /> Side Effects</div>
      ...
      {gate_state === "member" && compound.nutrition_label_url && (
        <div className="prohp-card p-6 mb-4">
          ...
          <img src={compound.nutrition_label_url} alt={(compound.name || "Supplement") + " supplement facts"} className="rounded-xl max-h-64 object-contain mx-auto cursor-pointer hover:opacity-80 transition-opacity" onClick={function() { setLabelOpen(true); }} />
      ...
      {/* Full article (member only) */}
      {gate_state === "member" && compound.article_content && (
        <div className="prohp-card p-6 mb-4"><div className="text-sm font-semibold text-slate-200 mb-2">Full Breakdown</div><ContentBlock content={compound.article_content} /></div>
      )}
      ...
      {gate_state === "lead" && compound.article_preview && (
        <div className="prohp-card p-6 mb-4 border border-[rgba(34,157,216,0.2)] bg-[rgba(34,157,216,0.04)]">
          ...
          <div className="flex items-center gap-2 mb-2"><Lock className="w-4 h-4 text-[#229DD8]" /><span className="text-sm font-bold text-slate-100">Want the full breakdown?</span></div>
          <p className="text-[13px] text-slate-400 mb-4">Dosing protocols, stacking logic, PCT, bloodwork markers.</p>
          <Link to="/register" className="inline-flex items-center justify-center rounded-xl bg-gradient-to-r from-[#0070f3] to-[#00c6ff] px-5 py-2.5 text-xs font-bold text-white shadow-lg hover:-translate-y-0.5 transition-all">Unlock Inner Circle</Link>
        </div>
      )}

      {/* ═══ 11. BLOODWORK CTA ═══ */}
      {gate_state === "member" && isSuppressive && <BloodworkCTA compoundName={compound.name} />}
      ...
      {compound && compound.thread_id && (
        <div id="community-discussion" className="prohp-card p-6 mb-4">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2"><MessageSquare className="w-4 h-4 text-prohp-400" /><div className="text-sm font-semibold text-slate-200">Community Discussion<span className="text-slate-500 font-normal ml-1.5">({threadPagination.total || 0} {threadPagination.total === 1 ? "reply" : "replies"})</span></div></div>
            <Link to={"/t/" + compound.thread_id} className="text-xs text-prohp-400 hover:text-prohp-300 transition-colors">View full discussion &rarr;</Link>
          </div>
          {threadQuery.isLoading ? (
            <div className="animate-pulse"><div className="h-16 bg-slate-800 rounded mb-2" /><div className="h-16 bg-slate-800 rounded mb-2" /><div className="h-16 bg-slate-800 rounded" /></div>
          ) : threadQuery.error ? (
            <div className="text-center py-4"><p className="text-xs text-slate-500">Discussion temporarily unavailable.</p><Link to={"/t/" + compound.thread_id} className="text-xs text-prohp-400 hover:text-prohp-300">View thread directly &rarr;</Link></div>
                  ...
                  <div key={post.id} className={"prohp-card px-4 py-3 " + (post.is_best_answer ? "border-l-2 border-l-emerald-500/50 bg-emerald-500/[0.03] " : "") + (post.parent_id ? "ml-8 border-l-2 border-l-slate-800/50" : "")}>
                    <div className="flex items-start gap-3">
                      <div className="flex flex-col items-center gap-0.5 min-w-[28px]">
                        <button onClick={function() { user && votePost046.mutate({ postId: post.id, value: 1 }); }} className={"p-0.5 transition-colors " + (!user ? "opacity-40 cursor-default" : "cursor-pointer") + " " + (post.user_vote === 1 ? "text-prohp-400" : "text-slate-600 hover:text-prohp-400")} disabled={!user}><ArrowUp className="w-3.5 h-3.5" /></button>
                        ...
                        <button onClick={function() { user && votePost046.mutate({ postId: post.id, value: -1 }); }} className={"p-0.5 transition-colors " + (!user ? "opacity-40 cursor-default" : "cursor-pointer") + " " + (post.user_vote === -1 ? "text-red-400" : "text-slate-600 hover:text-red-400")} disabled={!user}><ArrowDown className="w-3.5 h-3.5" /></button>
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 text-[11px] text-slate-500 flex-wrap">
                          ...
                          {gate_state === "member" && user && (<button onClick={function() { setReplyTo046(post.id); var el = document.getElementById("reply-box-046"); if (el) el.focus(); }} className="flex items-center gap-1 text-slate-500 hover:text-prohp-400 transition-colors ml-auto"><CornerDownRight className="w-3 h-3" /> Reply</button>)}
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          {user && gate_state === "member" ? (
            <div className="border-t border-white/[0.04] pt-4">
              {replyTo046 && (<div className="flex items-center gap-2 mb-2 text-xs text-slate-500"><CornerDownRight className="w-3 h-3" /><span>Replying to a post</span><button type="button" onClick={function() { setReplyTo046(null); }} className="text-slate-500 hover:text-slate-300 ml-1">cancel</button></div>)}
              ...
              <div className="flex items-center justify-between">
                ...
                <button type="submit" onClick={handleReply046} disabled={createReply046.isPending} className="prohp-btn-primary text-xs">{createReply046.isPending ? "Posting..." : "Post Reply"}</button>
              </div>
            </div>
          ) : null}
        </div>
      ...
      {communityStats && communityStats.total > 0 && (
        <div className="prohp-card p-6 mb-4 border border-prohp-400/15 bg-prohp-400/[0.03]">
          <h3 className="text-sm font-bold text-prohp-400 mb-4 flex items-center gap-2"><Shield className="w-4 h-4" /> Community Intel</h3>
          <div className="flex gap-3 flex-wrap mb-4">
            <div className="bg-prohp-400/10 rounded-lg px-3 py-2"><span className="text-[10px] text-slate-400 block">Total Reports</span><div className="text-lg font-bold text-white">{communityStats.total}</div></div>
            {communityStats.with_side_effects > 0 && (<div className="bg-red-900/20 rounded-lg px-3 py-2"><span className="text-[10px] text-slate-400 block">Side Effect Reports</span><div className="text-lg font-bold text-red-400">{communityStats.with_side_effects}</div></div>)}
          </div>
          {communityStats.top_side_effects && communityStats.top_side_effects.length > 0 && (<div className="flex gap-2 flex-wrap mb-4">{communityStats.top_side_effects.slice(0, 6).map(function(se, i) { return (<span key={i} className="bg-red-900/15 border border-red-700/25 rounded-full px-3 py-1 text-[11px] text-red-400">{se.effect} ({se.count})</span>); })}</div>)}
          {user && (user.tier === 'inner_circle' || user.tier === 'admin' || user.role === 'admin') ? (
            <div>{communityComments.length > 0 && (<div className="flex flex-col gap-2 mt-3"><h4 className="text-xs font-semibold text-slate-400">Top Community Comments</h4>{communityComments.map(function(c, i) { return (<div key={c.id || i} className="bg-white/[0.03] rounded-lg p-3 border border-white/5"><div className="text-[13px] text-slate-300 leading-relaxed mb-2">{c.content && c.content.length > 280 ? c.content.slice(0, 280) + '...' : c.content}</div><div className="flex justify-between text-[11px] text-slate-500"><span>{c.author || 'Anonymous'}</span><span className="text-prohp-400">{c.likes || 0} likes</span></div></div>); })}</div>)}</div>
          ) : (
            <div className="text-center pt-3 border-t border-white/5"><p className="text-xs text-slate-400 mb-3">Unlock full community intel: top comments, dosage patterns, and detailed reports</p><Link to="/register" className="prohp-btn-primary text-xs px-4 py-2">Unlock Community Intel</Link></div>
          )}
        </div>
      )}
