...
export default function CompoundDetail() {
  ...
  var [commentImage046, setCommentImage046] = useState(null);
  var [imagePreview046, setImagePreview046] = useState(null);
  var [uploading046, setUploading046] = useState(false);
  var imageInputRef046 = useRef(null);
  var uploadImage046 = async function(file) { var fd = new FormData(); fd.append("image", file); var tk = localStorage.getItem("prohp_at"); if (!tk) throw new Error("Login required"); var r = await fetch((import.meta.env.VITE_API_URL || "") + "/api/posts/upload", { method: "POST", headers: { "Authorization": "Bearer " + tk }, body: fd }); if (!r.ok) throw new Error("Upload failed"); return (await r.json()).url; };
  var handleImageSelect046 = function(e) { var f = e.target.files && e.target.files[0]; if (!f) return; var mx = f.type.startsWith("video/") ? 15728640 : 5242880; if (f.size > mx) { setReplyError046("File too large"); return; } setCommentImage046(f); setImagePreview046(URL.createObjectURL(f)); };
  var isVideo046 = function(url) { return url && (url.endsWith(".mp4") || url.endsWith(".webm") || url.endsWith(".mov")); };
  var deletePost046 = useMutation({ mutationFn: function(a) { return api.del("/api/posts/" + a.postId); }, onSuccess: function() { queryClient.invalidateQueries({ queryKey: ["compound-thread", compound ? compound.thread_id : null] }); } });
  ...
  var createReply046 = useMutation({ mutationFn: function(payload) { return api.post("/api/posts", payload); }, onSuccess: function() { queryClient.invalidateQueries({ queryKey: ["compound-thread", compound ? compound.thread_id : null] }); setReplyBody046(""); setReplyTo046(null); setReplyError046(""); setCommentImage046(null); setImagePreview046(null); if (imageInputRef046.current) imageInputRef046.current.value = ""; }, onError: function(err) { setReplyError046(err.message); } });
  var handleReply046 = function(e) { e.preventDefault(); if (!replyBody046.trim()) { setReplyError046("Say something."); return; } setReplyError046(""); (async function() { var imgUrl046 = null; if (commentImage046) { setUploading046(true); try { imgUrl046 = await uploadImage046(commentImage046); } finally { setUploading046(false); } } createReply046.mutate({ thread_id: compound.thread_id, body: replyBody046.trim(), parent_id: replyTo046 || undefined, image_url: imgUrl046 || undefined }); setCommentImage046(null); setImagePreview046(null); })(); };
